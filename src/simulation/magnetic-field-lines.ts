...
export class MagneticFieldLines {
  ...
  public static readonly CLOSED_LARGE_HIGH_ALTITUDE_RADIUS =
    Surface.GEOMETRY_RADIUS * 1.4;
  public static readonly OPEN_HIGH_ALTITUDE_RADIUS =
    Surface.GEOMETRY_RADIUS * 1.5;

  private static readonly LINE_POINT_LENGTH = 4;
  private static readonly CLOSED_SMALL_HIGH_ALTITUDE_RADIUS =
    Surface.GEOMETRY_RADIUS * 1.1;
  private static readonly CLOSED_MEDIUM_HIGH_ALTITUDE_RADIUS =
    Surface.GEOMETRY_RADIUS * 1.2;
  private static readonly CLOSED_LINE_RESOLUTION_RECIPROCAL =
    1 / (MagneticFieldLines.CLOSED_LINE_RESOLUTION - 1);
  ...
  private static readonly OPEN_LINE_RESOLUTION_RECIPROCAL =
    1 / (MagneticFieldLines.OPEN_LINE_RESOLUTION - 1);
  ...
  private static readonly CLOSED_LOW_ALTITUDE_STIFFNESS = Math.pow(
    Surface.GEOMETRY_RADIUS * 1.02,
    2,
  );
  private static readonly CLOSED_HIGH_ALTITUDE_STIFFNESS = Math.pow(
    Surface.GEOMETRY_RADIUS * 1.5,
    2,
  );
  ...
  public constructor(
    ...
    const closedBoundsLength =
      MagneticFieldLines.CLOSED_LINE_RESOLUTION * this.closedCount;
    this.closedLeftBounds = new StorageBufferAttribute(
      closedBoundsLength,
      MagneticFieldLines.LINE_POINT_LENGTH,
    );
    this.closedRightBounds = new StorageBufferAttribute(
      closedBoundsLength,
      MagneticFieldLines.LINE_POINT_LENGTH,
    );
    ...
    const openBoundsLength =
      MagneticFieldLines.OPEN_LINE_RESOLUTION * this.openCount;
    this.openLeftBounds = new StorageBufferAttribute(
      openBoundsLength,
      MagneticFieldLines.LINE_POINT_LENGTH,
    );
    this.openRightBounds = new StorageBufferAttribute(
      openBoundsLength,
      MagneticFieldLines.LINE_POINT_LENGTH,
    );
    this.computeOpen = Fn(() => {
      const connectionsBuffer = storage(
        magneticConnections.openConnectionsBuffer,
        'vec3',
      );
      const point = connectionsBuffer.element(instanceIndex).toVar();

      const rotationAxis = vec3(
        instanceIndex.mod(5).sub(2),
        instanceIndex.mod(7).sub(4),
        instanceIndex.mod(9).sub(4),
      )
        .normalize()
        .toVar();

      const leftPointLow = HelperFunctions.rotate(
        point,
        rotationAxis,
        MagneticFieldLines.OPEN_LOW_ANGLE,
      ).toVar();
      const rightPointLow = HelperFunctions.rotate(
        point,
        rotationAxis,
        -MagneticFieldLines.OPEN_LOW_ANGLE,
      ).toVar();
      ...
      const leftPointHigh = HelperFunctions.rotate(
        leftPointLow
          .normalize()
          .mul(MagneticFieldLines.OPEN_HIGH_ALTITUDE_RADIUS),
        rotationAxis,
      ...
      const pointIndex = instanceIndex
        .mul(MagneticFieldLines.OPEN_LINE_RESOLUTION)
        .toVar();
      Loop(MagneticFieldLines.OPEN_LINE_RESOLUTION, ({ i }) => {
        const progress = float(i)
          .mul(MagneticFieldLines.OPEN_LINE_RESOLUTION_RECIPROCAL)
          .toVar();
        const rightPoint = mix(rightPointLow, rightPointHigh, progress);
        const leftPoint = mix(leftPointLow, leftPointHigh, progress);
        const leftStiffness = leftPoint
          .lengthSq()
          .smoothstep(
            MagneticFieldLines.CLOSED_HIGH_ALTITUDE_STIFFNESS,
            MagneticFieldLines.CLOSED_LOW_ALTITUDE_STIFFNESS,
          );
        const rightStiffness = rightPoint
          .lengthSq()
          .smoothstep(
            MagneticFieldLines.CLOSED_HIGH_ALTITUDE_STIFFNESS,
            MagneticFieldLines.CLOSED_LOW_ALTITUDE_STIFFNESS,
          );
        storage(this.openLeftBounds, 'vec4')
          .element(pointIndex)
          .assign(vec4(leftPoint, leftStiffness));
        storage(this.openRightBounds, 'vec4')
          .element(pointIndex)
          .assign(vec4(rightPoint, rightStiffness));
  ...
  public async updateAsync(renderer: WebGPURenderer): Promise<void> {
    await Promise.all([
      renderer.computeAsync(
        this.computeSmallClosed().compute(
          this.magneticConnections.closedConnections.length,
        ),
      ),
      renderer.computeAsync(
        this.computeMediumClosed().compute(
          this.magneticConnections.closedConnections.length,
        ),
      ),
      renderer.computeAsync(
        this.computeLargeClosed().compute(
          this.magneticConnections.closedConnections.length,
        ),
      ),
  ...
  private createComputeClosed(
    ...
    return Fn(() => {
      ...
      const fourthPoint = connectionsBuffer
        .element(connectionId.add(1))
        .toVar();

      const rotationAxis = fourthPoint.sub(firstPoint).normalize().toVar();
      const rotationAngle = float(lineId)
        .mul(0.26)
        .fract()
        .mul(0.1)
        .add(minLineAngle)
      ...
      const secondPointLeft = HelperFunctions.rotate(
        firstPoint,
        rotationAxis,
        rotationAngle.negate(),
      )
        .normalize()
        .mul(
          float(lineId)
            .mul(0.31)
            .fract()
            .mul(MagneticFieldLines.CLOSED_LINE_HIGHT_VARIANCE)
            .add(highRadius),
        )
        .toVar();
      const secondPointRight = HelperFunctions.rotate(
        firstPoint,
        rotationAxis,
        rotationAngle,
      )
        ...
        .toVar();
      const thirdPointLeft = HelperFunctions.rotate(
        fourthPoint,
        rotationAxis,
        rotationAngle.negate(),
      )
        ...
        .toVar();
      const thirdPointRight = HelperFunctions.rotate(
        fourthPoint,
        rotationAxis,
        rotationAngle,
      )
      ...
      if (lineStretch !== undefined) {
        const stretchAxis = secondPointLeft
          .sub(thirdPointLeft)
          .normalize()
          .mul(lineStretch)
          .toVar();
        ...
        stretchAxis.assign(
          secondPointRight.sub(thirdPointRight).normalize().mul(lineStretch),
        );
        secondPointRight.addAssign(stretchAxis);
        thirdPointRight.addAssign(stretchAxis.negate());
      }

      const pointIndex = lineId
        .mul(MagneticFieldLines.CLOSED_LINE_RESOLUTION)
        .toVar();
      Loop(MagneticFieldLines.CLOSED_LINE_RESOLUTION, ({ i }) => {
        const progress = float(i)
          .mul(MagneticFieldLines.CLOSED_LINE_RESOLUTION_RECIPROCAL)
          .toVar();
        ...
        storage(this.closedLeftBounds, 'vec4')
          .element(pointIndex)
          .assign(vec4(leftPoint, leftStiffness));
        storage(this.closedRightBounds, 'vec4')
          .element(pointIndex)
          .assign(vec4(rightPoint, rightStiffness));
        pointIndex.addAssign(1);
      });

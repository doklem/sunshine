...
export class Surface
  extends Mesh<BufferGeometry, NodeMaterial>
  ...
  public constructor(
    ...
    super(
      new IcosahedronGeometry(
        Surface.GEOMETRY_RADIUS,
        Surface.GEOMETRY_DETAILS,
      ),
      new NodeMaterial(),
    );
    ...
    const convection = this.createFlowNode(
      convectionFlowAndSunspot.xyz,
      Surface.CONVECTION_SPEED,
      Surface.CONVECTION_ZOOM,
      voronoiTexture,
    );
    const halo = cameraPosition
      .normalize()
      .dot(normalWorld)
      .mul(Math.PI)
      .sin()
      .smoothstep(1, 0);
    const activity = this.createFlowNode(
      normalLocal,
      Surface.ACTIVITY_SPEED,
      Surface.ACTIVITY_ZOOM,
      simplexNoiseTexture,
    );
    ...
    this.renderHMIItensitygramColored = Fn(() => {
      const intensity = max(
        0,
        convection.mul(halo.mul(0.75).add(0.25)).sub(sunspot),
      );

...
export class DebugSprites extends InstancedMesh<PlaneGeometry, SpriteNodeMaterial> {
  ...
  public constructor(
    ...
    spriteSize? : number) {
    super(
      new PlaneGeometry(spriteSize ?? DebugSprites.DEFAULT_SPRITE_SIZE, spriteSize ?? DebugSprites.DEFAULT_SPRITE_SIZE, 1, 1),
      new SpriteNodeMaterial(),
      points.length
    );

    const positionBuffer = instancedArray(this.count, 'vec3');
    positionBuffer.value.set(new Float32Array(points.flatMap(point => point.toArray())));
    this.material.positionNode = positionBuffer.element(instanceIndex);    
    this.material.colorNode = colorNode;

    this.computeBoundingSphere();
    this.boundingSphere!.radius = radius ?? Surface.GEOMETRY_RADIUS;
  }

...
export abstract class FlaresBase
  extends InstancedMesh<PlaneGeometry, NodeMaterial>
  implements Configurable
{
  private static readonly DISPLACEMENT_SPEED = 0.05;
  private static readonly OFFSET_SPEED = 0.6;

  protected static readonly SURFACE_RADIUS_SQUARED =
    Surface.GEOMETRY_RADIUS * Surface.GEOMETRY_RADIUS;
  ...
  protected constructor(
    ...
    super(FlaresBase.createGeometry(resolution), new NodeMaterial(), count);
    ...
    const pointIndex = positionLocal.x
      .add(instanceIndex.mul(resolution.x))
      .toVar();
    const leftLinePoint = storage(leftBounds, 'vec4')
      .element(pointIndex)
      .toVar();
    const rightLinePoint = storage(rightBounds, 'vec4')
      .element(pointIndex)
      .toVar();
    ...
    const stiffness = mix(
      leftLinePoint.a,
      rightLinePoint.a,
      positionBetweenLines,
    );
    const displacement = texture(
      vertexNoise,
      vec2(time.mul(FlaresBase.DISPLACEMENT_SPEED), lineId),
    ).xyz.mul(stiffness.oneMinus());
    const position = mix(
      leftLinePoint.xyz,
      rightLinePoint.xyz,
      positionBetweenLines,
    ...
    this.material.colorNode = Fn(() => {
      ...
      const intensity = heightSq
        .smoothstep(
          FlaresBase.SURFACE_RADIUS_SQUARED * 1.2,
          FlaresBase.SURFACE_RADIUS_SQUARED,
        )
        .add(alpha.mul(1));
  ...
  protected abstract createHightMask(
  ...
  private static createGeometry(resolution: Vector2): PlaneGeometry {
    ...
    return new PlaneGeometry(width, 1, width, resolution.y).translate(
      width * 0.5,
      0,
      0,
    );
